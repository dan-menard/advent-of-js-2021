(() => {
  ...
  function buildDOM() {
    ...
    const episodeTemplate = document.querySelector('template#episode');

    episodes.forEach((episode) => {
      const templateContent = episodeTemplate.content.cloneNode(true);
      const episodeEl = templateContent.querySelector('li');

      const episodeIdString = `episode-${episode.id}`
      episodeEl.querySelector('label').htmlFor = episodeIdString;
      episodeEl.querySelector('input').id = episodeIdString;
      episodeEl.querySelector('input').name = episodeIdString;

      const episodeTitleString = `${episode.id} || ${episode.name}`
      episodeEl.querySelector('span').textContent = episodeTitleString;
